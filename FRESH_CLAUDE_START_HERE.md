...
- **Linux = viewer-only, always.** `pygetwindow` is effectively Windows-only — it does not enumerate windows on Linux regardless of display server (Wayland *or* X11, Cinnamon *or* GNOME). When the ICQ runs on Linux it drops to **viewer-only** mode automatically: it prints incoming messages to its own terminal and Nir copy-pastes them by hand into Claude Code. No crash, no silent auto-type failure. The ICQ will NOT show a numbered window list on Linux and that is expected — do not try to "fix" it by installing extra packages. Outbound messages from Claude (curl POST `/send`) still work normally in both directions.
